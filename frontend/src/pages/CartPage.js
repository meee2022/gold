...
function CartPage() {
  ...
  const handleCheckout = async () => {
    try {
      await apiCall("post", "/orders", { items: cart.items, coupon_code: couponCode });
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#050505] pb-20">
        <TopHeader title="سلة التسوق" showBack={true} showCart={false} />
        <div className="flex flex-col items-center justify-center h-[60vh] px-4">
          ...
          <Button onClick={function() { navigate("/auth"); }} className="bg-[#D4AF37] hover:bg-[#F4C430] text-black rounded-full">
            تسجيل الدخول
          </Button>
        </div>
        <BottomNav />
  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-screen bg-[#050505] pb-20" data-testid="cart-page">
        <TopHeader title="سلة التسوق" showBack={true} showCart={false} />
        <div className="flex flex-col items-center justify-center h-[50vh] px-4">
          <ShoppingBag size={64} className="text-[#27272A] mb-4" />
          <p className="text-[#A1A1AA] mb-4">سلة التسوق فارغة</p>
          <Button onClick={function() { navigate("/store"); }} className="bg-[#D4AF37] hover:bg-[#F4C430] text-black rounded-full">
            تسوق الآن
          </Button>
        </div>
        <BottomNav />
    ...
    <div className="min-h-screen bg-[#050505] pb-32" data-testid="cart-page">
      ...
      <div className="px-4 py-2">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="text-white">{cartTotal.toLocaleString()} ر.ق</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="border-t border-[#27272A] pt-2 mt-2">
            <div className="flex justify-between">
              ...
              <span className="text-[#D4AF37] font-bold text-lg">{cartTotal.toLocaleString()} ر.ق</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#0A0A0A] border-t border-[#27272A] p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <span className="text-[#A1A1AA] text-sm">إجمالي العناصر ({cartItems.length})</span>
            <p className="text-[#D4AF37] font-bold">{cartTotal.toLocaleString()} ر.ق</p>
          </div>
